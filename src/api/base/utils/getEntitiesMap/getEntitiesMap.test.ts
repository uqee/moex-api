...
import { getEntitiesMap } from './getEntitiesMap'

describe('getEntitiesMap', () => {
  const dto: Dto = {
    metadata: {},
  ...
  test('works', () => {
    const enginesArray: Engine[] = getEntitiesArray(dto)
    const enginesMap: Map<Engine['id'], Engine> = getEntitiesMap(enginesArray)
